...
async function testXpcFix(articleId) {
  console.log("\n=========================");
  console.log("Testing Xpc Fix for articleId:", articleId);

  // 1. 测试 https://mod-api.xinpianchang.com/v2/article/${articleId}?app_key=61a60037a34e0
  ...
  try {
    ...
    console.log("JSON status_code:", json.status_code);
    if (json.data) {
...
async function main() {
  await testXpcFix("13777600");
  await testXpcFix("13776982");
}
